...
<div class="panel-content" dir="rtl" style="padding: 1.5rem;">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
            <h4 class="m-0">⚙️ إدارة المميزات والباقات</h4>
            <p class="text-muted small m-0 mt-1">
                التحكّم في المميزات المتاحة لكل باقة — تأثير فوري على كل المستخدمين
            </p>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0" id="featuresMatrix">
                    ...
                    <tbody>
                        ...
                        @foreach($features as $feature)
                                        ...
                                        <span class="float-end">
                                            @foreach($plans as $plan)
                                                <button type="button" 
                                                    ...
                                                    onclick="toggleCategory('{{ $feature->category }}', {{ $plan->id }}, true)"
                                                    title="تفعيل كل ميزات هذي الفئة لـ{{ $plan->name }}">
                                                    {{ $plan->name }} ✓
                                                </button>
                                            @endforeach
                                        </span>
                            ...
                            <tr>
                                ...
                                @foreach($plans as $plan)
                                    @php
                                        $isEnabled = $matrix[$feature->id][$plan->id] ?? false;
                                    @endphp
...
const toggleCategoryUrl = "{{ route('admin.features.toggle.category') }}";
const csrfToken = "{{ csrf_token() }}";

// تفعيل/إلغاء ميزة واحدة
...
// تفعيل فئة كاملة
async function toggleCategory(category, planId, isEnabled) {
    if (!confirm('هل تريد تفعيل كل ميزات هذي الفئة لـهذي الباقة؟')) return;

    try {
        const response = await fetch(toggleCategoryUrl, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken,
        ...
        const data = await response.json();
        if (data.success) {
            showToast('تمّ التحديث للفئة', 'success');
            location.reload();
        }
    } catch (e) {
        showToast('خطأ', 'error');
