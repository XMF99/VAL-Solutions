...
class ManageUsersController extends Controller
{
    ...
    protected function userData(mixed $scope = 'query')
    {
        if ($scope != 'staff') {
    ...
    public function detail($id)
    {
        ...
        $pricingPlans = SubscriptionPlan::active()->get();
    ...
    {
        $request->validate([
            'product_limit'   => 'required|integer|gte:-1',
            'user_limit'      => 'required|integer|gte:-1',
            'warehouse_limit' => 'required|integer|gte:-1',
            'supplier_limit'  => 'required|integer|gte:-1',
            'coupon_limit'    => 'required|integer|gte:-1',
        ...
        $lastPurchase = $user->planPurchases()->orderBy('id', 'desc')->first();
        ...
        $user->warehouse_limit = $request->warehouse_limit;
        $user->supplier_limit  = $request->supplier_limit;
        $user->coupon_limit    = $request->coupon_limit;
        $user->hrm_access      = $request->hrm_access ? Status::YES : Status::NO;
        ...
        notify($user, 'SUBSCRIPTION_EXTENDED', [
            ...
            'warehouse_limit' => $user->warehouse_limit,
            'supplier_limit'  => $user->supplier_limit,
            'coupon_limit'    => $user->coupon_limit,
            'hrm_access'      => $user->hrm_access ? "<span style='color:green;'>Yes</span>" : "<span style='color:red;'>No</span></span>",
        ]);
    ...
    public function assignSubscription(Request $request, $id)
    {
        ...
        $user        = User::findOrFail($id);
        $pricingPlan = SubscriptionPlan::findOrFail($request->plan_id);
        ...
        $planPurchase->recurring_type       = $request->plan_recurring;
        $planPurchase->amount               = getPurchasePrice($pricingPlan, $request->plan_recurring);
        $planPurchase->payment_method       = Status::ADMIN_PAYMENT;
        $planPurchase->gateway_method_code  = 0;
        $planPurchase->subscription_plan_id = $pricingPlan->id;
        $planPurchase->auto_renewal         = Status::NO;
        $planPurchase->expired_at           = $expireAt;
        $planPurchase->save();

        $user->plan_id         = $pricingPlan->id;
        $user->product_limit   = $pricingPlan->product_limit == -1 ? -1 : $user->product_limit + $pricingPlan->product_limit;
        $user->user_limit      = $pricingPlan->user_limit == -1 ? -1 : $user->user_limit + $pricingPlan->user_limit;
        $user->warehouse_limit = $pricingPlan->warehouse_limit == -1 ? -1 : $user->warehouse_limit + $pricingPlan->warehouse_limit;
        $user->supplier_limit  = $pricingPlan->supplier_limit == -1 ? -1 : $user->supplier_limit + $pricingPlan->supplier_limit;
        $user->coupon_limit    = $pricingPlan->coupon_limit == -1 ? -1 : $user->coupon_limit + $pricingPlan->coupon_limit;
        $user->hrm_access      = $pricingPlan->hrm_access;
